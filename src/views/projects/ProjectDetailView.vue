...
// L'id est passé automatiquement par le router grâce à "props: true" dans la config
...
const fetchProjectData = async () => {
  ...
  try {
    // 1. Détails du projet
    const projectData = await api.get(`projects/${props.id}`);
    project.value = projectData;

    // 2. Tâches (Kanban)
    const tasksData = await api.get(`projects/${props.id}/tasks`);
    tasks.value = tasksData;

    // 3. Statistiques
    const statsData = await api.get(`dashboard/projects/${props.id}/stats`);
...
// Navigation retour via le routeur
const goBack = () => {
  router.push({ name: 'projects' });
};

// Organisation des tâches pour le Kanban
...
<template>
  <div class="p-8">
    <!-- Bouton Retour : Utilise maintenant le router -->
    <button @click="goBack"
    ...
    <div v-if="loading" class="text-center p-20 text-xl font-semibold text-indigo-600">
    ...
    <div v-else-if="error" class="text-center p-20 text-red-700 bg-red-100 rounded-xl border border-red-300">
      <h2 class="text-2xl font-bold mb-3">Erreur</h2>
      <p>{{ error }}</p>
    </div>
    ...
    <div v-else-if="project" class="space-y-10">
      ...
      <div class="bg-white p-8 rounded-xl shadow-lg border-t-4 border-indigo-600">
        ...
        <p class="text-gray-500 mb-6">Propriétaire : {{ project.ownerEmail }}</p>

        <!-- FIX : Ajout de la section Description qui manquait -->
        <div class="mb-6">
          <h2 class="text-xl font-semibold text-indigo-600 mb-2">Description :</h2>
          <p class="text-gray-700 whitespace-pre-line bg-gray-50 p-4 rounded-lg border">
      ...
      <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="bg-gray-100 p-4 rounded-xl">
            <h3 class="font-bold text-gray-700 mb-4 uppercase tracking-wider">À FAIRE ({{ tasksByStatus.TO_DO.length }})</h3>
            <div v-for="task in tasksByStatus.TO_DO" :key="task.id" class="bg-white p-4 rounded-lg shadow-sm mb-3 border-l-4 border-gray-400">
          ...
          <!-- EN COURS -->
          <div class="bg-gray-100 p-4 rounded-xl">
            <h3 class="font-bold text-gray-700 mb-4 uppercase tracking-wider">EN COURS ({{ tasksByStatus.IN_PROGRESS.length }})</h3>
            <div v-for="task in tasksByStatus.IN_PROGRESS" :key="task.id" class="bg-white p-4 rounded-lg shadow-sm mb-3 border-l-4 border-orange-500">
              <p class="font-medium text-gray-800">{{ task.title }}</p>
            </div>
          </div>
          ...
          <!-- TERMINÉ -->
          <div class="bg-gray-100 p-4 rounded-xl">
            <h3 class="font-bold text-gray-700 mb-4 uppercase tracking-wider">TERMINÉ ({{ tasksByStatus.DONE.length }})</h3>
            <div v-for="task in tasksByStatus.DONE" :key="task.id" class="bg-white p-4 rounded-lg shadow-sm mb-3 border-l-4 border-green-500">
              <p class="font-medium text-gray-800">{{ task.title }}</p>
            </div>
          </div>
        </div>
